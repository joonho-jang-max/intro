...
function BenefitBanner() {
  return (
    <div style={{ position: 'relative' }}>
      <img src={`${BASE}figma/cookie2.png`} style={{ display: 'block', width: '100%' }} alt="웰컴 쿠키 선물"/>
      <span style={{
        position: 'absolute', top: 12, left: 12,
        background: DARK, color: WHITE, fontSize: 11, fontWeight: 800, padding: '3px 8px', borderRadius: 4,
      }}>NEW</span>
      <span style={{
        position: 'absolute', top: 12, right: 12,
        background: '#ff2d78', color: WHITE, fontSize: 11, fontWeight: 800, padding: '3px 12px', borderRadius: 20,
      }}>참여하기</span>
      <span style={{
        position: 'absolute', bottom: 10, right: 12,
        background: 'rgba(80,40,130,0.45)', color: WHITE, fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 10,
      }}>2/14 ›</span>
    </div>
  )
}
    ...
    <div style={{ width: 106, flexShrink: 0 }}>
      ...
      <div style={{ width: 106, height: 148, borderRadius: 8, overflow: 'hidden', position: 'relative', marginBottom: 8 }}>
        ...
        <img src={`${BASE}figma/badge_cup.png`}
          style={{ position: 'absolute', top: 2, right: 2, width: 22, height: 28 }}/>

        {/* 시리즈에디션 뱃지 (우하단 코너) */}
        <img src={`${BASE}figma/badge_series.png`}
          style={{ position: 'absolute', bottom: 18, right: 0, width: 36, height: 36 }}/>

        {/* 특가세트 뱃지 (하단 바) */}
        <img src={`${BASE}figma/badge_specialprice.png`}
          style={{ position: 'absolute', bottom: 0, left: 0, height: 22, width: 'auto' }}/>
      </div>

      {/* NEW 뱃지 + 타이틀 */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 4 }}>
        <img src={`${BASE}figma/badge_new.png`} style={{ height: 14, width: 'auto' }}/>
  ...
  return (
    <div style={{ background: LIGHT_BG, paddingTop: 18, paddingBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px', marginBottom: 14 }}>
        <span style={{ color: DARK, fontSize: 15 }}><b>닉네임</b> 님이 좋아할 만한 작품</span>
...
export default function Component() {
  ...
  return (
    // 반응형: 화면 전체를 채우되 콘텐츠는 375px로 센터링
    <div style={{ background: '#222', minHeight: '100vh', display: 'flex', justifyContent: 'center' }}>
      <div style={{
